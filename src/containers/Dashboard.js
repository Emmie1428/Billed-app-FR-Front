...
export default class {
  ...
  handleShowTickets(e, bills, index) {
    if (this.ticketStatus === undefined) this.ticketStatus = {};
  
    this.ticketStatus[index] = !this.ticketStatus[index];
    const isOpen = this.ticketStatus[index];
    
    if (isOpen) {
        $(`#arrow-icon${index}`).css({ transform: 'rotate(0deg)'});
        $(`#status-bills-container${index}`)
        .html(cards(filteredBills(bills, getStatus(index))));    
        this.attachBillListeners(bills);
    } else {
